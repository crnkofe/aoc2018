...
(defn -main
  ...
  (with-open [rdr (clojure.java.io/reader (nth args 0))]
    (let [sorted-guard (sort #(date-compare %1 %2) (line-seq rdr))
          ...
          sleep-guard (guards-sleep-time parsed-guard)
          max-val (apply max (vals sleep-guard))
          sleepy-guard (first (keys (filter #(= (second %) max-val) sleep-guard)))
          frequent-minute (most-frequent-min (get parsed-guard sleepy-guard))
          sleeped-most-on-minute (apply max (vals frequent-minute))
          ]
      (println (* (Integer. sleepy-guard) (first (keys (filter #(= (second %) sleeped-most-on-minute) frequent-minute)))))
  )))
